...
-   Adding custom containers to the deployment
...
A more advanced configuration with can be achieved via the ``docker-compose.override.yml`` file. A template is generated by cookiecutter and can be copied from `docker-compose.override.yml.template` to `docker-compose.override.yml` for modifying your local Dioptra instance.
...
For this example, let's assume that you have a docker image named `custom-container` with a `dev` tag that you would like to integrate into your dioptra deployment.

To add custom containers to the deployment, add the following code the `services:` section of the ``docker-compose.override.yml`` file:
